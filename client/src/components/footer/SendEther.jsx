...
import { FiSend, FiCheckCircle, FiAlertCircle, FiExternalLink, FiCopy } from 'react-icons/fi';
import { getBlockExplorerUrl, getBlockExplorerName } from '../../utils/blockExplorers';
...
const SendEther = () => {
  ...
  const calculateUSDT = () => {
    if (!amount || parseFloat(amount) <= 0 || !ethPrice || !usdtPrice) return null;
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-6">
            ...
            {isConnected && (
              ...
              >
                <span className="text-xs font-semibold text-green-500 dark:text-green-400">Connected</span>
              </motion.div>
            )}
          </div>
          ...
          >
              ...
              {amount && parseFloat(amount) > 0 && (
                ...
                >
                  <div className="flex items-center justify-between p-3 bg-indigo-500/10 dark:bg-indigo-500/5 rounded-lg border border-indigo-500/20">
                    <span className="text-sm font-medium text-ternary-dark dark:text-ternary-light">USD Value:</span>
                    <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">
                      ${ethPrice ? (parseFloat(amount) * ethPrice).toFixed(2) : '...'}
                    </span>
                  </div>
                  {usdtAmount && (
                    <div className="flex items-center justify-between p-3 bg-green-500/10 dark:bg-green-500/5 rounded-lg border border-green-500/20">
                      <span className="text-sm font-medium text-ternary-dark dark:text-ternary-light">USDT Equivalent:</span>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <motion.button
                      ...
                      title="Copy address"
                    >
                      <FiCopy className={`text-sm ${copied ? 'text-green-500' : 'text-indigo-500 dark:text-indigo-400'}`} />
                    </motion.button>
                  </div>
                </div>
              </div>
            ...
            {success && (
              ...
              >
                ...
                <div className="flex-1">
                  <span className="text-sm font-semibold block mb-1">Transaction successful! 🎉</span>
                  {hash && explorerUrl && (
                    <a
                      href={explorerUrl}
                      target="_blank"
            ...
            <motion.button
              onClick={handleSend}
              disabled={isSending || isConfirming || !amount || parseFloat(amount) <= 0}
              whileHover={!isSending && !isConfirming && amount && parseFloat(amount) > 0 ? { scale: 1.02 } : {}}
              whileTap={!isSending && !isConfirming && amount && parseFloat(amount) > 0 ? { scale: 0.98 } : {}}
